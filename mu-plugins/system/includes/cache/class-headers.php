...
final class Headers {
	...
	public function remove_extra_cookies() {
		...
		$cookies = array_filter( array_keys( $_COOKIE ) );
		...
		$cookies = array_merge(
			preg_grep( '/^wp-settings-\d+/', $cookies ),
			preg_grep( '/^wp-settings-time-\d+/', $cookies )
		);

		foreach ( $cookies as $cookie ) {

			setcookie( $cookie, null, 0, SITECOOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	...
	public function wp_headers( $headers ) {
		...
		if ( ! $nocache_headers_present && ! is_user_logged_in() ) {
